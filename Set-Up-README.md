...
## Overview
- **Product name:** `cdp_curated_sap_domain1`
- **Layer:** `curated`
- *Domain:** `domain1`
...
The CLI that ran to create this repo performed the following actions for the given `cdp_curated_sap_domain1` and `curated`:
...
- Created an **Account level group** with read access for consumers: `cdp-curated-cdp_curated_sap_domain1-readers`. This group is intended to be the standard consumer group for granting read access to the product in UC.
...
- **`id`** *(Required)* — unique technical name of the data product (e.g., cdp_curated_sap_domain1). Was set automatically!
...
- **Access & Consumer Group** *(Required)* — group in the format cdp-curated-cdp_curated_sap_domain1-readers, description of how to request access.
...
- Group created: `cdp-curated-cdp_curated_sap_domain1-readers` — add consumers here to grant read access to UC objects for this product.
...
- [ ] Confirm that `cdp-curated-cdp_curated_sap_domain1-readers` group exists and members know how to request access
